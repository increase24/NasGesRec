...
class NAgeStureNet(nn.Module):
    def __init__(self, C8, C16, C32, num_classes, layers, genotype):
        super(NAgeStureNet, self).__init__()
        self._C8 = C8  # 48
        self._C16 = C16  # 32
        self._C32 = C32  # 16
        ...
        for i in range(layers):
            if i in [3 ,6]: # dimension reduction
                C_curr8 *= 2
                reduction_prev = True
                if i == 3:
    ...
    def forward(self, inputs): # inputs: 3*32*112*112
        # 32-frame path
        s0_32 = self.stem0_32(inputs) # 16*32*56*56
        s1_32 = self.stem1_32(s0_32) # 32*16*28*28
        s0_32 = self.AvgpoolSpa(s0_32) # 16*16*28*28
        s1_32_lateral_16_1 = self.lateral32_16_1(s1_32) # 32*8*28*28
        s1_32_lateral_8_1 = self.lateral32_8_1(s1_32) # 32*4*28*28

        # 16-frame path
        s0_16 = self.stem0_16(inputs[:, :, ::2, :, :]) # 16*16*56*56
        s1_16 = self.stem1_16(s0_16) # 32*8*28*28
        s0_16 = self.AvgpoolSpa(s0_16) # 16*8*28*28
        s1_16_lateral_8_1 = self.lateral16_8_1(s1_16) # # 32*4*28*28
        s1_16 = torch.cat([s1_32_lateral_16_1, s1_16], dim=1) # (32+32)*8*28*28
        
        # 8-frame path
        s0_8 = self.stem0_8(inputs[:, :, ::4, :, :])  # 16*8*56*56
        s1_8 = self.stem1_8(s0_8)  # 32*4*28*28
        s0_8 = self.AvgpoolSpa(s0_8)  # 16*4*28*28
        s1_8 = torch.cat([s1_32_lateral_8_1, s1_16_lateral_8_1, s1_8], dim=1) # (32+32+32)*4*28*28

        # 32
        for idx, cell in enumerate(self.cells32):
            s0_32, s1_32 = s1_32, cell(s0_32, s1_32)
            if idx == 2: # the third cell
                s1_32 = self.MaxpoolSpa(s1_32)
                s1_32_lateral_16_2 = self.lateral32_16_2(s1_32)
                s1_32_lateral_8_2 = self.lateral32_8_2(s1_32)
            elif idx == 5: # the 6th cell
                s1_32 = self.MaxpoolSpa(s1_32)
                s1_32_lateral_16_3 = self.lateral32_16_3(s1_32)
                s1_32_lateral_8_3 = self.lateral32_8_3(s1_32)
        
        # 16
        for idx, cell in enumerate(self.cells16):
            s0_16, s1_16 = s1_16, cell(s0_16, s1_16)
            if idx == 2: # the third cell
                s1_16 = self.MaxpoolSpa(s1_16)
                s1_16_lateral_8_2 = self.lateral16_8_2(s1_16)
                s1_16 = torch.cat([s1_32_lateral_16_2, s1_16], dim=1)
            elif idx == 5: # the 6th cell
                s1_16 = self.MaxpoolSpa(s1_16)
                s1_16_lateral_8_3 = self.lateral16_8_3(s1_16)
                s1_16 = torch.cat([s1_32_lateral_16_3, s1_16], dim=1)

        # 8
        for idx, cell in enumerate(self.cells8):
            s0_8, s1_8 = s1_8, cell(s0_8, s1_8)
            if idx == 2: # the third cell
                s1_8 = self.MaxpoolSpa(s1_8)
                s1_8 = torch.cat([s1_32_lateral_8_2, s1_16_lateral_8_2, s1_8], dim=1)
            elif idx == 5: # the 6th cell
                s1_8 = self.MaxpoolSpa(s1_8)
                s1_8 = torch.cat([s1_32_lateral_8_3, s1_16_lateral_8_3, s1_8], dim=1)

        out_32 = self.global_pooling(s1_32)
        out_16 = self.global_pooling(s1_16)
        out_8 = self.global_pooling(s1_8)
        out = torch.cat([out_32, out_16, out_8], dim = 1)
        logits = self.classifier(self.dropout(out.view(out.size(0), -1)))
        return logits
...
if __name__ == '__main__':
    ...
    model = NAgeStureNet(init_channels8, init_channels16, init_channels32, classes, layers, genotype_RGB)
    print(model)
